...
public class YiyangParser implements DriverParser {
	@Override
			...
			if (table != null) {
				...
				if (trs.size() > 1) {
					...
					for (int i = 0; i < trs.size(); i++) {
						...
						if (status.contains("已处罚")) {
							continue;
						}
						n++;
						jcase = new JSONObject();
						jcase.put("序号", String.valueOf(n));
						String time = tds.get(2).text().substring(1) + tds.get(3).text();
						String dizhi = tds.get(6).text().substring(1);

						String con = tds.get(7).text().substring(1);
						jcase.put("处理情况", "未处理");
						jcase.put("违法时间", time);
