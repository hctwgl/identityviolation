...
public class KaifengParser implements DriverParser {
	@Override
	public String parse(String msg,DriverProfile driverProfile) {
		...
		try {
			...
			Elements divs = doc.getElementsByClass("content1");
			if (divs.size() > 0) {
				Elements tables = divs.get(1).getElementsByTag("table");
