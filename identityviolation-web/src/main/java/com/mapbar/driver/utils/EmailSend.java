...
public class EmailSend {
	...
	public static void Mail(String subject, String day) {
		// 创建邮件信息
		MultiMailSenderInfo mailInfo = new MultiMailSenderInfo();
		mailInfo.setMailServerHost("mail.mapbar.com");
		mailInfo.setMailServerPort("25");
		mailInfo.setValidate(true);

		mailInfo.setUserName(ConfigUtil.MailName);
		mailInfo.setPassword(ConfigUtil.MailPassword);
		mailInfo.setFromAddress(ConfigUtil.MailName);
		mailInfo.setToAddress(ConfigUtil.MailName);
		String[] Ccs = ConfigUtil.Ccs.split(",");
		mailInfo.setCcs(Ccs);
		mailInfo.setSubject(subject);
		mailInfo.setContent(day);

		// String[] receivers = new String[]{"wangfan2@mapbar.com"};//添加其他收件人
		// String[] ccs = receivers;
		// mailInfo.setReceivers(receivers);
		// mailInfo.setCcs(ccs);

		EmailSend.sendMailtoMultiReceiver(mailInfo);
		EmailSend.sendMailtoMultiCC(mailInfo);
	}
